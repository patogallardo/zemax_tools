...
for conf_num in progressbar(range(1, MCE.NumberOfConfigurations+1)):
    MFE.RemoveOperandsAt(1, MFE.NumberOfOperands)

    rows_to_diff = []
    MFE.AddOperand()
    MFE.GetOperandAt(1).GetOperandCell(2).IntegerValue = conf_num
    MCE.SetCurrentConfiguration(conf_num)
    ...
    MF_fnameout = "adjust_circular_stop_MF_env_%i.MF" % (conf_num)
    MF_fnameout = os.path.join(OUTPUTDIR, MF_fnameout)
    MF_fnameout = os.path.abspath(MF_fnameout)
