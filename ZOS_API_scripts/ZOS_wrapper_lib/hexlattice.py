...
def plot_df(df, scale, show=True):
    ...
    for j in range(len(df)):
        ...
        s_conf = "%i" % (df.config.values[j])  # configs start at 1
        x, y = df.x.values[j], df.y.values[j]
        plt.text(x, y, s_conf, fontsize=8, color='white', ha='center',
                 va='center')

    ax.set_aspect('equal')
#    plt.scatter(df.x, df.y, color='white')
    plt.xlim(1.11 * np.array([df.x.min(), df.x.max()]))
    plt.ylim(1.11 * np.array([df.y.min(), df.y.max()]))
