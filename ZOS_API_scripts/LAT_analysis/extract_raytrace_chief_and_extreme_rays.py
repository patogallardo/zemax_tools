''' From TMA design, export rays from normalized sky coordinates to focal
plane coordinates.

This script will use field position 1 to get Hx, Hy.
'''

from win32com.client.gencache import EnsureDispatch, EnsureModule  # noqa
...
if __name__ == '__main__':
    ...
    TheSystem.SystemData.Fields.Normalization = 0  # force radial norm
...
# center around first field
    ...
    print('CenterField x: %1.3f, CenterField y: %1.3f' % (center_field_x_deg,
                                                          center_field_y_deg))
    print('Max Field: %1.3f' % max_field)

# define field positions
#    assert delta_xy_deg < max_field # check that deltaxydeg is within max
...
#    max_wave = TheSystem.SystemData.Wavelengths.NumberOfWavelengths
    ...
    x_ary = np.empty([len(pxpys)*len(hx_arr)])  # center field +4 fields
