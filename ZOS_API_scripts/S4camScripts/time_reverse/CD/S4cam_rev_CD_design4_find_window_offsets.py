...
for currentconf in progressbar(range(1, NCONF + 1)):
    ...
    op.GetOperandCell(2).IntegerValue = int(currentconf)

    # add REAX operand
    op = MFE.GetOperandAt(2)
    op.ChangeType(ZOSAPI.Editors.MFE.MeritOperandType.REAX)
    op.GetOperandCell(2).IntegerValue = int(window_entrance_surface)
