...
if __name__ == '__main__':
    ...
    for wavelengthNumber in range(1, 3+1):
        ...
        # at which angle does it hit the edge of the array at 65mm?
        normalization_angle = get_norm_angle(Fields)

        radii_array = np.zeros(nconf)
        for current_conf in range(1, nconf+1):
            MCE.SetCurrentConfiguration(current_conf)
            radii_array[current_conf-1] = fsolve(f, 1) * normalization_angle
