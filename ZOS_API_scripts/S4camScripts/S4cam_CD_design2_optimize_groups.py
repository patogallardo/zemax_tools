...
def change_surfaceTypes(surface_numbers, to_type, LDE, ZOSAPI):
    for surface_number in surface_numbers:
        srf = LDE.GetSurfaceAt(surface_number)
        surf_type = srf.GetSurfaceTypeSettings(to_type)  # noqa
        srf.ChangeType(surf_type)


def copy_std_lens_params_to_biconic(lens_surface_numbers,
                                    current_conf, MCE, LDE,
                                    biconic_cfg_numbers=[[16, 17, 18],
                                                         [27, 28, 29],
                                                         [38, 39, 40]]):
    assert len(lens_surface_numbers) == len(biconic_cfg_numbers)
    for j in range(len(lens_surface_numbers)):
        lens_surface_number = lens_surface_numbers[j]
        s = LDE.GetSurfaceAt(lens_surface_number)
        R = s.Radius
        C = s.Conic
        Angle = 0.0
...
for j, group_leader in progressbar(enumerate(group_leaders)):
    ...
    if STANDARD_SURF_ON_FIRST_IT:  # change surface to standard
        to_type = ZOSAPI.Editors.LDE.SurfaceType.Standard
        change_surfaceTypes(SURFACES_TO_START_AS_STANDARD,
                            to_type, LDE, ZOSAPI)
    ...
    if RUN_OPTIMIZER and RUN_OPTIMIZER2ndRound:
        if STANDARD_SURF_ON_FIRST_IT:  # now turn it back to biconic
            to_type = ZOSAPI.Editors.LDE.SurfaceType.Biconic
            change_surfaceTypes(SURFACES_TO_END_AS_BICONIC,
                                to_type, LDE, ZOSAPI)

            copy_std_lens_params_to_biconic(SURFACES_TO_END_AS_BICONIC,
                                            conf_to_vary, mce, LDE,
                            biconic_cfg_numbers=BICONIC_CFG_NUMBERS)  # noqa
