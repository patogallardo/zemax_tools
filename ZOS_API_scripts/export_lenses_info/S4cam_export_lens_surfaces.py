...
#   identify mirror names and surface numbers
    lens_first_surf, lens_curved_surfaces, lens_names = zmx.get_lens_surfaces(TheSystem)  # noqa
    lens_surfaces = lens_curved_surfaces
    is_curved_first = [lens_first_surf[j] == lens_curved_surfaces[j] for
                       j in range(3)]
    ...
    for jlens, lensSurface in enumerate(lens_surfaces):  # iterate over mirrors
        ...
        for col in range(1, Ncol):  # iterate over columns
            ...
            if "(unused)" not in c.Header:
                ...
                if c.DataType == 0:
                    ...
                    colval = c.Value
                # do this, except for the thickness of second lens
                if c.Header == "Thickness" and not is_curved_first[jlens]:
                    aux_surf = TheLDE.GetSurfaceAt(lens_first_surf[jlens])
                    colval = aux_surf.GetSurfaceCell(col).DoubleValue
